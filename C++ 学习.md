...
- #### 默认拷贝构造函数
     ...
     A(const A & ATemp)，拷贝构造函数不会对静态成员变量进行处理

     默认拷贝构造函数是浅拷贝，如果对象存在动态成员，会在析构函数对同个动态变量进行2次delete
...
- #### RTTI
     ...
     1. dynamic_cast操作符，作用：将派生类指针转换为基类指针，确保可以安全调用虚函数

        ```c++
        dynamic_cast<Type *> (pt)
        ```

        如果指向的对象(*pt) 的类型为Type或者从Type直接或间接派生而来的类型，则表达式将指针pt转换为Type类型的指针；否则返回结果0，即空指针

     2. typeid操作符返回一个type_info对象
...
- #### auto_ptr
     ...
     问题：new Class 会在对应~Class()析构函数进行释放内存，但是对于常规指针，程序很容易在new出来，但是没有执行delete，导致内存泄露

     解决：引入auto_ptr，auto_ptr模板定义了类似指针对象，可以将new获得（直接/间接）的地址赋给这种对象。当auto_ptr对象过期时，其析构函数将使用delete来释放内存。
     ...
     /*
     #1、为变量pd、double值分配存储空间
              pd                 double
     地址：    4000               10000	
     值：      10000              25.5
     函数运行完之后，局部变量pd会删除，但是值还是保存在动态内存中
     */
     void demo2()
     {
     ...
     */
     ...
     - auto_ptr模板使用的是delete，所以不能与new[]一起使用

     - ```c++
       auto_ptr<string> ps (new string ("hello c++."));
       ...
       问题在于vocation和ps指针都是指向同一个string对象，程序将试图删除同一个对象两次
